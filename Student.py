...
class Student(Human):
    ...
    def get_class(self):
        return self.__class
    def __repr__(self):
        return f"Student(name='{self.name}', last_name='{self.last_name}', id='{self.id}')"
    def __str__(self):
        if self.__class is not None:
            return f"Student: {self.name} {self.last_name}, id: {self.id}, Class: {self.__class._grade} {self.__class._letter}"
        else:
